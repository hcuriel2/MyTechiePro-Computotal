...
export class ServiceTechnicianSelectComponent implements OnInit {
  ...
  public techSelectDetails(pro: User): void {
    ...
    this.router.navigateByUrl("/proDetails", {
      state: { pro, categoryId, serviceId, serviceName },
    });
  ...
  notifyAdmin(): void {
    this.authService
      .notifyAdmin(this.clientName, this.clientEmail, this.skill)
      .subscribe({
        next: (response) => {
          this.snackBar.open("The admin has been notified!", "Close", {
            duration: 3000,
          });
        },
        error: (error) => {
  ...
  private async getStaticData() {
    return new Promise((resolve, reject) => {
      forkJoin([
        this.userService.getAllProfessionalsBySkill(
          ...
          map(([users, categories]) => {
            for (let i = 0; i < users.length; i++) {
              if (users[i].ratingSum == 0 || users[i].ratingSum == null) {
                ...
                users[i].rating = (
                  users[i].ratingSum / users[i].ratingCount
                ).toFixed(2);
              }
            }
  ...
  public displayTechs() {
    ...
    if (this.professionals.length === 0) {
      this.searchFeedback =
        "No Technicians found, please change distance radius or contact Admin";
      this.isFeedbackVisible = true;
    }
  }
  ...
  public async getAreaCode() {
    try {
      ...
      this.http.get(url).subscribe((res: any) => {
        ...
        this.areaCode = addressData[addressData.length - 1].long_name.split(
          " ",
          1
        );
      });
